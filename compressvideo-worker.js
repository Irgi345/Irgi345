/* ================================================================
   ...
   Version : 6.5.3 — SAME-ORIGIN WORKER FIX
   ================================================================
   [ROOT CAUSE FIX]
     Browser memblokir `new Worker(cdnURL)` karena worker script
     WAJIB same-origin. FFmpeg.wasm 0.12.x membuat worker internal
     dari CDN → diblokir.

   [FIX]
     1. Fetch worker.js dari CDN sebagai text
     2. Rewrite relative imports (./classes.js, ./const.js, ...)
        menjadi absolute CDN URLs
     3. Bungkus hasilnya sebagai Blob → dapat Blob URL (same-origin)
     4. Pass via option `classWorkerURL` ke FFmpeg.load()
   ================================================================ */

'use strict';

const WORKER_VERSION = '6.5.3';
...
/** Fetch text dari URL dengan timeout. */
...
/** Fetch binary dari URL sebagai blob URL. */
...
/**
 * Fetch FFmpeg ESM worker.js dan rewrite relative imports menjadi
 * absolute CDN URLs, lalu bungkus sebagai Blob URL (same-origin).
 */
async function createClassWorkerBlobURL(base) {
  ...
  // Rewrite: from "./classes.js"  →  from "https://.../classes.js"
  const fixed = workerSrc.replace(
    /(\bfrom\s*)(["'])\.\/([^"']+)\2/g,
    (_, pre, q, file) => pre + q + esmBase + '/' + file + q
  );

  // Juga handle dynamic import("./xxx.js")
...
async function loadFFmpegFrom(base) {
  ...
  /* 1) Fetch worker.js dan rewrite imports-nya */
  const classWorkerURL = await createClassWorkerBlobURL(base);

  /* 2) Fetch core JS & WASM sebagai blob URLs */
  ...
  /* 3) Import FFmpeg class (module import, tidak diblokir) */
  ...
  /* 4) Construct FFmpeg dengan classWorkerURL yang sudah same-origin */
