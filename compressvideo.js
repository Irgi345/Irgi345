/* ================================================================
   ...
   Version : 6.5.0 — PRECISE / FEATURE-COMPLETE
   ================================================================
   Changelog v6.5:
     [NEW] Preload worker + FFmpeg saat boot (auto)
     [NEW] Tab navigation (Dasar / Output / Transformasi / Lanjutan)
     [NEW] Output format: MP4 / WebM / MKV / GIF / MP3 / M4A / Thumbnail
     [NEW] Batch mode (queue + ZIP via JSZip)
     [NEW] Split-view before/after dengan slider
     [NEW] Analisis video (codec, fps, tracks) via ffmpeg -i
     [NEW] Preset custom (simpan ke localStorage)
     [NEW] Riwayat lanjutan (re-apply, hapus individual, export JSON)
     [NEW] Estimasi vs Aktual badge
     [NEW] Error mapping FFmpeg → pesan manusiawi
     [NEW] Debounce estimate, throttle progress, RAF chart
     [NEW] Cache refs DOM saat boot
   ================================================================ */
...
const WORKER_VERSION_EXPECTED = '6.5.0';
const HISTORY_KEY        = 'irgxy_cv_history_v65';
const LEGACY_HISTORY_KEY = 'irgxy_cv_history_v61';
const CUSTOM_PRESETS_KEY = 'irgxy_cv_presets_v65';
...
function mapFFmpegError(msg) {
  ...
  if (m.includes('coop_coep') || m.includes('sharedarraybuffer')) {
    return 'Server tidak mengaktifkan COOP/COEP. Jalankan lewat HTTPS dengan header Cross-Origin-Isolation.';
  }
  if (m.includes('memory') || m.includes('oom') || m.includes('out of memory')) {
    return 'Memori tidak cukup. Coba resolusi lebih rendah atau file lebih kecil.';
  }
  if (m.includes('exit code 1') || m.includes('exit code 234')) {
    return 'File input rusak atau codec tidak didukung FFmpeg.';
  }
  if (m.includes('timeout'))    return 'Proses timeout — file terlalu besar untuk perangkat ini.';
  if (m.includes('network') || m.includes('http ')) return 'Gagal mengunduh FFmpeg — cek koneksi internet.';
...
function updateEstimate() {
  ...
  if (fmt === 'gif' || fmt === 'mp3' || fmt === 'm4a' || fmt === 'thumbnail') {
    /* Estimasi kasar untuk mode khusus */
    const dur = effectiveDuration() || 1;
    let est = 0;
    if (fmt === 'gif')       est = dur * 12 * 300 * 1024 / 8;
    else if (fmt === 'mp3')  est = (audioBitrateKbps() || 128) * dur * 1000 / 8;
    else if (fmt === 'm4a')  est = (audioBitrateKbps() || 128) * dur * 1000 / 8;
    else if (fmt === 'thumbnail') est = 80 * 1024;
...
/* ================================================================
   [13] BUILD ARGS (dengan output format)
   ================================================================ */
...
function buildVideoArgs(container) {
  ...
  if (container === 'webm') {
    ...
    if (method === 'quality') {
      args.push('-crf', String(clamp(safeNum((refs.slider || {}).value, 30), 0, 63)));
      args.push('-b:v', '0');
    } else {
    ...
    if (method === 'quality') {
      args.push('-crf', String(clamp(safeNum((refs.slider || {}).value, 23), 0, 51)));
    } else {
      const kbps = calcVideoBitrate();
  ...
  if (res && res !== 'keep' && state.metadata.width > 0) {
    ...
    if (tw && th) {
      if (tw > state.metadata.width) filters.push('scale=' + state.metadata.width + ':' + state.metadata.height);
      else filters.push('scale=' + tw + ':' + th + ':force_original_aspect_ratio=decrease');
    }
  }
  ...
  if (wmText) {
    ...
    const posMap = {
      ...
      tc: ['(w-tw)/2', '10'], bc: ['(w-tw)/2', 'h-th-10']
    };
    ...
    filters.push("drawtext=text='" + escaped + "':x=" + pos[0] + ':y=' + pos[1] +
                 ':fontsize=' + size + ':fontcolor=' + color + box);
  }
  ...
  if (container === 'webm') {
    ...
    else args.push('-c:a', ac === 'libopus' ? 'libopus' : 'libopus', '-b:a', getAudioBitrate() + 'k', '-ac', '2');
...
function buildAudioOnlyArgs(format) {
  ...
  args.push('-vn');
  if (format === 'mp3') {
    args.push('-c:a', 'libmp3lame', '-b:a', getAudioBitrate() + 'k', '-ac', '2');
  } else {
...
function buildThumbnailArgs() {
  ...
  const args = ['-hide_banner', '-ss', atSec.toFixed(3), '-i', '__INPUT__',
                '-frames:v', '1', '-q:v', '3', '__OUTPUT__'];
  return args;
}
  ...
  try {
    ...
    switch (d.type) {
      ...
      case 'status':
        ...
        if (d.stage === 'core-downloading' && d.message) setWorkerStatus(d.message, 'idle');
        else if (d.stage === 'wasm-compiling') setWorkerStatus(d.message, 'idle');
        else if (d.stage === 'loading') setWorkerStatus(d.message, 'idle');
        break;
      case 'progress':
...
function refreshStartButton() {
  ...
  if (state.processing) {
    ...
    btn.innerHTML = '<i class="fas fa-hourglass-half"></i> Menyiapkan FFmpeg...';
  } else {
    btn.innerHTML = '<i class="fas fa-play"></i> MULAI COMPRESS';
  }
}
...
async function handleFile(file) {
  ...
  try {
    ...
    /* Auto file-type warning */
    const ext = (file.name.split('.').pop() || '').toLowerCase();
    if (ext === 'mkv') toast('MKV → output MP4 akan di-remux, mungkin lebih lama.', 'info', 5000);
    if (ext === 'webm') toast('WebM dengan audio copy → otomatis fallback ke AAC.', 'info', 5000);
...
async function startCompression() {
  ...
  if (method === 'quality') {
    const v = safeNum((refs.slider || {}).value, -1);
    if (v < 0 || v > 51) { toast('CRF harus 0–51.', 'error'); return; }
  }
  ...
  try {
    ...
    worker.postMessage({
      ...
      payload: {
        fileBuffer,
        config: Object.assign({}, state.lastConfig, {
          effectiveDuration: state.lastConfig.duration
        })
      }
    });
  } catch (err) {
...
function handleWorkerDone(d) {
  ...
  setText('#savedText', savedPct > 0
    ...
    : '⚠ Ukuran output tidak lebih kecil. Coba turunkan bitrate/CRF.');
  ...
  saveHistory({
    ...
    codec: cfg.codec, method: cfg.method,
    outputFormat: cfg.outputFormat || 'mp4',
    config: {
      codec: cfg.codec, method: cfg.method, resolution: cfg.resolution,
      audioCodec: cfg.audioCodec, audioBitrate: cfg.audioBitrate, target: cfg.target
    },
    date: Date.now(), savedPct
  });

  /* Auto-show split-view */
  if (state.previewUrl && state.resultUrl) {
    showSplitView();
  }
}
...
function showAccuracyBadge(cfg, actualBytes) {
  ...
  el.innerHTML = '<i class="fas fa-crosshairs"></i> Akurasi estimasi: ' + acc + '%';
}
...
function bindSplitView() {
  ...
  range.addEventListener('input', () => {
    const pct = safeNum(range.value, 50);
...
function captureCurrentSettings() {
  return {
    ...
    target: getMethod() === 'size-mb' ? safeNum((refs.numberInput || {}).value, 50) :
            safeNum((refs.slider || {}).value, 50),
...
function addCustomPreset() {
  ...
  const cfg = captureCurrentSettings();
  state.customPresets = state.customPresets.filter(p => p.name !== name);
  state.customPresets.push({ name, cfg, date: Date.now() });
...
function renderCustomPresets() {
  ...
  host.innerHTML = state.customPresets.map(p =>
    '<div class="cv-preset-custom-row">' +
      '<button type="button" class="cv-preset-apply" data-name="' + escapeHtml(p.name) + '">' +
        '<i class="fas fa-star"></i> ' + escapeHtml(p.name) +
      '</button>' +
      '<button type="button" class="cv-preset-remove" data-name="' + escapeHtml(p.name) + '" aria-label="Hapus">' +
...
function migrateHistory() {
  try {
    ...
    if (legacy && !localStorage.getItem(HISTORY_KEY)) {
      ...
      console.info('[migrate] history v61 → v65');
    }
  } catch (_) {}
}
  ...
  list.innerHTML = items.map((h, i) =>
    '<div class="cv-history-item">' +
      ...
      '<div class="hi-content">' +
        ...
        ' · ' + new Date(h.date || Date.now()).toLocaleString('id-ID') + '</div>' +
      '</div>' +
  ...
  host.innerHTML = state.batch.map((b, i) =>
    '<div class="cv-batch-item">' +
      ...
      '<div class="bi-content">' +
        '<div class="bi-name">' + escapeHtml(b.file.name) + '</div>' +
        '<div class="bi-meta">' + formatBytes(b.file.size) + ' · ' + b.status + '</div>' +
        '<div class="bi-bar"><div class="bi-bar-fill" style="width:' + b.progress + '%"></div></div>' +
      '</div>' +
...
async function analyzeVideo() {
  ...
  try {
    ...
    const result = await new Promise((resolve) => {
      const handler = (e) => {
        const d = e.data || {};
        if (d.type === 'status' && d.message) {
          if (/Stream #|Duration|Input #/.test(d.message)) {
            analysisLines.push(d.message);
          }
        }
      ...
      const analysisLines = [];
      state.worker.addEventListener('message', handler);
      /* Run ffmpeg -i input (akan exit error, tapi log lengkap) */
      state.worker.postMessage({
        type: 'compress',
        payload: {
          fileBuffer: buf,
...
function onKeydown(e) {
  ...
  if (e.key === 'o' || e.key === 'O') { e.preventDefault(); $('#fileInput').click(); }
    ...
    if (state.resultUrl) { e.preventDefault(); const dl = refs.btnDownload; if (dl) dl.click(); }
  }
...
function bindEvents() {
  ...
  window.addEventListener('beforeunload', (e) => {
    if (state.processing) {
      e.preventDefault();
      e.returnValue = 'Proses sedang berjalan. Yakin keluar?';
...
function boot() {
  try {
    ...
    ['videoPreview','resultVideo','logBody','logPanel','workerStatus','workerStatusText',
     'workerVersionTag','btnStart','progressFill','progressBar','toastContainer',
     'fileInput','batchInput','dropzone','slider','numberInput','sliderWrap','numberWrap',
     ...
     'historyCard','historyList','settingsCard','helpModal'].forEach(id => {
      refs[id] = document.getElementById(id);
    });
    refs.sliderWrap = $('#sliderWrap');
    refs.numberWrap = $('#numberWrap');
    refs.dynamicLabel = $('#dynamicLabel');
    refs.workerStatus = $('#workerStatus');
    refs.workerStatusText = $('#workerStatusText');
    refs.workerVersionTag = $('#workerVersionTag');
    refs.btnStart = $('#btnStart');
    ...
    if (coiWarn && !self.crossOriginIsolated) coiWarn.style.display = 'block';

    /* Preload worker + FFmpeg */
    ensureWorker();
    setWorkerStatus('Memuat worker...', 'idle');

    console.log('%c✅ compressvideo.js v' + APP_VERSION + ' siap', 'color:#43e97b;font-weight:bold');
  } catch (e) {
