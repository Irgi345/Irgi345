...
const WORKER_VERSION_EXPECTED = '6.5.2';
...
function mapFFmpegError(msg) {
  ...
  if (m.includes('network') || m.includes('http ') || m.includes('fetch')) return 'Gagal mengunduh FFmpeg — cek koneksi internet.';
...
function boot() {
  try {
    ...
    if (coiWarn) coiWarn.style.display = 'none';  /* single-thread: tidak butuh COOP/COEP */
