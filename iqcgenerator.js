/* ================================================================
   ...
   Version: 5.14 (Home Indicator Removed + Rounded Box + Overflow Fix)
   ================================================================
   CHANGELOG v5.14:
     [FIX] renderContextMenuPreview: tambah container.style.overflowX='hidden'
     [FIX] onclone force .iqc-cm-reactions border-radius:12px
     [FIX] onclone force .iqc-cm-reactions max-width:100% + flex-wrap:wrap
     [FIX] onclone force .iqc-cm-reactions box-sizing:border-box
     [FIX] Post-render rAF force border-radius & layout reactions
     [KEEP] Inline Timestamp di pojok kanan bubble (v5.13)
     [KEEP] Instagram iOS fully removed (v5.12)
     [KEEP] Custom Emoji Image + Size Slider (v5.11)
   ================================================================ */
(function () {
  'use strict';
  ...
  const REACTIONS_BORDER_RADIUS = '12px'; /* v5.14 — kotak rounded, bukan oval */
  ...
  function applyEmojiSizeStyles(reactionsEl, size) {
    ...
    /* v5.14 — Pastikan border-radius tetap kotak rounded + cegah overflow */
    reactionsEl.style.borderRadius = REACTIONS_BORDER_RADIUS;
    reactionsEl.style.maxWidth = '100%';
    reactionsEl.style.boxSizing = 'border-box';
    reactionsEl.style.flexWrap = 'wrap';
  }
  ...
  /* ================================================================
     RENDER CONTEXT MENU (single mode) — INLINE TIMESTAMP + BOX ROUNDED
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.14 — overflow-x:hidden cegah konten keluar horizontal */
    ...
    const bubbleHTML = `
      <div class="iqc-cm-bubble ${msgType}" style="background:${bubbleColor};color:${textColor};">
        <span class="iqc-cm-bubble-content">${escapeHtml(msgText)}</span><span class="iqc-cm-bubble-spacer" aria-hidden="true"></span>
    ...
    /* Post-render force: pastikan #444444 & border-radius kotak menang dari cache */
    requestAnimationFrame(() => {
      ...
      if (reactionsEl) {
        ...
        /* v5.14 — Force kotak rounded (bukan oval) */
        reactionsEl.style.borderRadius = REACTIONS_BORDER_RADIUS;
        reactionsEl.style.maxWidth = '100%';
        reactionsEl.style.boxSizing = 'border-box';
        reactionsEl.style.flexWrap = 'wrap';
        reactionsEl.style.setProperty('border-radius', REACTIONS_BORDER_RADIUS, 'important');
      }
  ...
  function preloadContextMenuImage() {
    return new Promise((resolve) => {
      ...
      img.onload  = () => { console.log('[IQC v5.14] CM image loaded:', url); finish(); };
      img.onerror = () => { console.warn('[IQC v5.14] CM image gagal load:', url); finish(); };
  ...
  function preloadCustomEmojiImages() {
    ...
    return Promise.all(urls.map(url => new Promise((resolve) => {
      ...
      let done = false;
      const finish = () => { if (done) return; done = true; resolve(); };
      img.onload  = () => { console.log('[IQC v5.14] Emoji loaded:', url); finish(); };
      img.onerror = () => { console.warn('[IQC v5.14] Emoji gagal load:', url); finish(); };
      img.src = url;
      ...
      return await html2canvas(frame, {
        ...
        onclone: (clonedDoc) => {
          try {
            ...
            if (clonedStage) {
              clonedStage.style.zIndex = '2';
              clonedStage.style.overflow = 'hidden';
              clonedStage.style.boxSizing = 'border-box';
            }

            /* ── 4) Force bubble & reactions background #444444 + kotak rounded ── */
            ...
            clonedDoc.querySelectorAll('.iqc-cm-reactions').forEach(el => {
              ...
              /* v5.14 — kotak rounded, bukan oval */
              el.style.borderRadius = REACTIONS_BORDER_RADIUS;
              el.style.setProperty('border-radius', REACTIONS_BORDER_RADIUS, 'important');
            ...
            clonedDoc.querySelectorAll('.iqc-cm-plus, .iqc-cm-bubble-time, .iqc-cm-bubble-spacer').forEach(el => {
              el.style.fontFamily = `${TEXT_FONT}, ${EMOJI_FONT}`;
            ...
            /* ── 10) Force inline timestamp layout ── */
            clonedDoc.querySelectorAll('.iqc-cm-bubble').forEach(el => {
            ...
            clonedDoc.querySelectorAll('.iqc-cm-bubble-spacer').forEach(el => {
              el.style.display = 'inline-block';
              el.style.width = '54px';
              el.style.height = '0';
              el.style.fontSize = '0';
            ...
            clonedDoc.querySelectorAll('.iqc-cm-bubble-time').forEach(el => {
            ...
            /* ── 11) Kill animations & force sizes ── */
            ...
            killStyle.textContent = `
              *, *::before, *::after {
              ...
              .iqc-cm-bubble-content { display: inline !important; }
              .iqc-cm-bubble-spacer {
                display: inline-block !important;
              ...
              .iqc-cm-reactions {
                background: ${BUBBLE_COLOR} !important;
                border-radius: ${REACTIONS_BORDER_RADIUS} !important;
              ...
              /* v5.14 — hapus total home indicator di capture */
              .iqc-home-indicator { display: none !important; }
            `;
            clonedDoc.head.appendChild(killStyle);

            /* ── 12) Reset padding chat-messages ── */
            ...
            if (clonedMessages) {
              ...
              clonedMessages.style.overflowX = 'hidden';
            }
          } catch (e) { console.warn('[IQC onclone v5.14]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  function init() {
    try {
      ...
      console.log('✅ IQC Generator v5.14 (Home Indicator Removed + Rounded Box) siap');
    } catch (err) {
