/* ================================================================
   ...
   Version: 5.5 (FINAL — 100% Presisi, Konsisten Preview↔Download)
   ================================================================
   CHANGELOG v5.5:
     - Force hide notch/status bar di clone html2canvas (double-lock)
     - Fix z-index layering (blur → dim → stage)
     - Presisi bubble hug-content (inline-flex + flex-wrap)
     - Warna match iOS: #1F1F1F bubble, #FF453A danger
     - Menu items: Balas, Teruskan, Salin, Beri bintang, Hapus, Lainnya
     - Background image URL imagekit (cover, center, no-repeat)
...
(function () {
  ...
  function doRenderPreview() {
    ...
    if (State.mode === 'contextmenu') {
      ...
      frame.classList.remove('cm-mode'); // restore notch & status bar di mode IG
    }
  ...
  /* ================================================================
     RENDER CONTEXT MENU — v5.5
     Notch & status bar HIDDEN via class "cm-mode" (inline di HTML)
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    frame.classList.add('cm-mode'); // double-lock: hide notch + status bar
    ...
    /* Fake blurred bubbles (background) */
    const fakeBubbles = [
      { type:'in',  text:'Halo' },
      ...
      onclone: (clonedDoc) => {
        try {
          ...
          if (State.mode === 'contextmenu') {
            ...
            // Inject style untuk pseudo-element notch
          ...
          /* === Bakcup blur & dim === */
          const clonedBlurredBg = clonedDoc.querySelector('.iqc-cm-blurred-bg');
          if (clonedBlurredBg) {
            const blurPx = clamp(parseInt(State.contextmenu.blurAmount, 10) || 18, 5, 30);
            clonedBlurredBg.style.filter = `blur(${blurPx}px)`;
            clonedBlurredBg.style.transform = 'scale(1.05)';
            clonedBlurredBg.style.zIndex = '0';
          }
  ...
  function init() {
    try {
      ...
      console.log('✅ IQC Generator v5.5 (FINAL) siap');
    } catch (err) {
