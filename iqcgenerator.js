/* ================================================================
   IQC GENERATOR — iqcgenerator.js (v5.1 — BG IMAGE + OVERLAY)
   ================================================================
   Fitur:
     ...
     [4] ★ Background image dari URL + Overlay jam/baterai editable
   ================================================================ */
(function () {
  'use strict';
  ...
  /* ★ BARU — Konstanta background image URL */
  const BG_IMAGE_URL = 'https://ik.imagekit.io/IrgxyMods/2383af11-2091-4331-9887-6627d8b71d7c.png';
  ...
  const State = {
    ...
    /* ★ BARU — metadata gambar background */
    bgImage: null,
    bgImageLoaded: false,
  ...
  /* ================= ★ BARU — STATUS OVERLAY UPDATE ================= */
  function updateStatusOverlay() {
    try {
      const timeEl = document.getElementById('iqcStatusTimeOverlay');
      const batteryEl = document.getElementById('iqcStatusBatteryOverlay');
      const overlay = document.getElementById('iqcStatusOverlay');

      if (!timeEl && !batteryEl) return;

      /* Ambil nilai dari contextmenu (mode WhatsApp) */
      const cm = State.contextmenu || DEFAULT_CM;
      const timeValue = (cm.statusTime && /^\d{1,2}:\d{2}$/.test(cm.statusTime))
        ? cm.statusTime : '16:00';
      const batteryValue = clamp(parseInt(cm.statusBattery, 10) || 0, 0, 100);

      if (timeEl) timeEl.textContent = timeValue;
      if (batteryEl) batteryEl.textContent = batteryValue + '%';

      /* Sembunyikan overlay jika status bar dimatikan */
      if (overlay) {
        overlay.style.display = State.statusBar === false ? 'none' : 'flex';
      }
    } catch (e) {
      console.warn('[IQC updateStatusOverlay]', e);
    }
  }

  /* ================= ★ BARU — PRELOAD BACKGROUND IMAGE ================= */
  function preloadBackgroundImage() {
    try {
      const img = new Image();
      img.crossOrigin = 'anonymous';  /* untuk CORS html2canvas */
      img.onload = function () {
        console.log('[IQC] Background image loaded:', this.naturalWidth, 'x', this.naturalHeight);
        State.bgImage = img;
        State.bgImageLoaded = true;
        State.bgImageNaturalWidth  = this.naturalWidth;
        State.bgImageNaturalHeight = this.naturalHeight;
        /* Pastikan CSS background juga di-set konsisten */
        const frame = document.getElementById('iqcPhoneFrame');
        if (frame) {
          frame.style.backgroundImage = `url('${BG_IMAGE_URL}')`;
      ...
      img.onerror = function () {
        console.warn('[IQC] Background image failed to load, using fallback gradient');
        State.bgImageLoaded = false;
        const frame = document.getElementById('iqcPhoneFrame');
        if (frame) {
          frame.style.backgroundImage = 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)';
          frame.style.backgroundColor = '#1a1a2e';
        }
      };
      img.src = BG_IMAGE_URL;
    } catch (e) {
      console.warn('[IQC preloadBackgroundImage]', e);
    }
  }
  ...
  function doRenderPreview() {
    ...
    if (State.mode === 'contextmenu') {
      ...
      if (chatHeader) chatHeader.style.display = 'none'; /* tetap hidden karena ada di gambar */
    }
    ...
    if (statusBar) {
      statusBar.style.display = 'none'; /* ★ hidden karena ada di gambar */
    }
    ...
    /* ★ UBAH: chatBg background dibuat transparan agar background frame terlihat */
    chatBg.style.backgroundImage = '';
    chatBg.style.backgroundSize  = '';
    chatBg.style.backgroundPosition = '';
    chatBg.style.filter = '';
    chatBg.style.background = 'transparent';   /* ★ BARU — ganti dari #000/#FFF */
    ...
    /* ★ Update overlay setelah semua render */
    updateStatusOverlay();
  }
  ...
  function renderContextMenuPreview() {
    ...
    /* ★ UBAH: chatBg dibuat transparan agar background frame terlihat */
    chatBg.style.background = 'transparent';   /* ★ BARU */
    chatBg.style.backgroundImage = '';
  ...
  function initToggleSwitches() {
    const bind = (id, key) => {
      ...
      el.addEventListener('change', () => {
        ...
        renderPreview(); savePrefs();
        /* ★ Update overlay jika status bar toggle diubah */
        if (key === 'statusBar') updateStatusOverlay();
      });
    };
  ...
  function initContextMenuForm() {
    ...
    const bindText = (id, key, transform) => {
      ...
      el.addEventListener('input', () => {
        try {
          ...
          renderPreview();
          savePrefs();
          /* ★ Update overlay jika statusTime diubah */
          if (key === 'statusTime') updateStatusOverlay();
    ...
    const bindSlider = (id, valId, key, transform) => {
      ...
      el.addEventListener('input', () => {
        try {
          ...
          renderPreview(); savePrefs();
          /* ★ Update overlay jika battery diubah */
          if (key === 'statusBattery') updateStatusOverlay();
    ...
    /* ★ BARU — Panggil update overlay pertama kali */
    updateStatusOverlay();
  }
  ...
  async function captureCanvas(scale = 2) {
    ...
    return await html2canvas(frame, {
      ...
      onclone: (clonedDoc) => {
        try {
          /* ★ Pastikan overlay status bar tetap terlihat saat capture */
          const clonedOverlay = clonedDoc.getElementById('iqcStatusOverlay');
          if (clonedOverlay) {
            clonedOverlay.style.display = State.statusBar === false ? 'none' : 'flex';
          }
          /* ★ Pastikan background image tetap ter-render */
          const clonedFrame = clonedDoc.getElementById('iqcPhoneFrame');
          if (clonedFrame && State.bgImageLoaded) {
            clonedFrame.style.backgroundImage = `url('${BG_IMAGE_URL}')`;
            clonedFrame.style.backgroundSize = 'cover';
            clonedFrame.style.backgroundPosition = 'center center';
          }
  ...
  function init() {
    try {
      /* ★ Preload background image sebelum render */
      ...
      renderPreview();
      renderMessageList();
      /* ★ Update overlay setelah init */
      updateStatusOverlay();

      if (window.AOS && typeof AOS.init === 'function') {
        try { AOS.init({ duration: 650, easing: 'ease-out-expo', once: true, offset: 30 }); } catch (e) {}
      }
      console.log('✅ IQC Generator v5.1 (BG Image + Overlay editable) siap');
    } catch (err) {
  ...
  window.IQC = {
    ...
    updateStatusOverlay, preloadBackgroundImage
  };
})();