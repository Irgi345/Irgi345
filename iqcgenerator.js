/* ================================================================
   ...
   Version: 5.8 (PIXEL-PERFECT — Preview = Download 100%)
   ================================================================
   CHANGELOG v5.8:
     [FIX #1] Context Menu icon: ganti Font Awesome <i> → <img>
              dari URL ImageKit (BackgroundEraser_20260922_111514161.png)
     [FIX #1] Preload gambar ikon (preloadContextMenuIcons) sebelum
              html2canvas dieksekusi — anti blank icon saat export
     [FIX #1] onclone: paksa rendering <img> (width/height/object-fit)
     [FIX #2] imageTimeout dinaikkan ke 30000ms (30 detik) untuk
              antisipasi koneksi lambat saat unduh gambar ikon
     [FIX #3] Force anti-aliasing (font-smoothing, text-rendering)
              pada teks menu & bubble
     [FIX #4] Force solid background pada .iqc-cm-menu (html2canvas
              tidak mendukung backdrop-filter)
     [FIX #5] Hapus menuIcons map (tidak lagi pakai Font Awesome)
   ================================================================ */
(function () {
  'use strict';
  ...
  /* v5.8 FIX: URL gambar ikon context menu (WAJIB — dipakai untuk semua item) */
  ...
  /* ================================================================
     RENDER CONTEXT MENU — v5.8 (IMAGE ICONS)
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.8 FIX: ganti <i class="fas ..."> → <img src="CM_ICON_URL"> */
  ...
  /* ================================================================
     ...
     Sangat penting: gambar ikon harus di-cache browser sebelum
     html2canvas dijalankan, jika tidak ikon akan blank.
     ================================================================ */
  function preloadContextMenuIcons() {
    return new Promise((resolve) => {
      ...
      img.onload = () => { console.log('[IQC v5.8] CM icon loaded:', CM_ICON_URL); finish(); };
      img.onerror = () => { console.warn('[IQC v5.8] CM icon gagal load:', CM_ICON_URL); finish(); };
      img.src = CM_ICON_URL;
  ...
  /* ================================================================
     v5.8 FIX — CAPTURE CANVAS (PIXEL-PERFECT)
     ================================================================ */
  async function captureCanvas(scale = 3) {
    ...
    /* v5.8: preload bg + ikon context menu secara paralel */
    ...
    try {
      return await html2canvas(frame, {
        ...
        logging: false,
        /* v5.8: timeout dinaikkan 20s → 30s */
        imageTimeout: 30000,
        ...
        onclone: (clonedDoc) => {
          try {
            ...
            clonedDoc.querySelectorAll('.iqc-cm-item img').forEach(img => {
              ...
              /* v5.8: high-quality pada scale besar agar tidak blur */
            ...
            /* v5.8: pastikan .iqc-cm-item layout flex tetap konsisten */
            ...
            killStyle.textContent = `
              *, *::before, *::after {
              ...
              /* v5.8: paksa ikon context menu tampil mulus */
              .iqc-cm-item img {
          ...
          } catch (e) { console.warn('[IQC onclone v5.8]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  /* ================================================================
     v5.8 FIX — DOWNLOAD PNG (scale 3 / HD 4, retry 2x, quality 1.0)
     ================================================================ */
  ...
  function init() {
    try {
      ...
      /* v5.8: preload ikon context menu lebih awal (masuk browser cache) */
      ...
      console.log('✅ IQC Generator v5.8 (Pixel-Perfect + Image Icons) siap');
    } catch (err) {
