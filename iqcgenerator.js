/* ================================================================
   IQC GENERATOR — iqcgenerator.js (v2.0 — STABLE)
   ================================================================
   Perbaikan utama dari v1.0:
     [1] Memory-safe: semua ObjectURL di-revoke, audio node di-disconnect
     [2] Cancel bersih: abort signal dicek di setiap tahap
     [3] Avatar onerror anti-loop (flag guard)
     [4] Progress smooth (EMA + FPS real)
     [5] probeVideo TIDAK return URL yg sudah direvoke (bug kritis v1.0)
     [6] MediaRecorder mime detection diperluas + reporting format ke user
     [7] Guard dobel-RAF + debounce race
     [8] Semua handler try/catch
     [9] Cleanup on visibilitychange + beforeunload
    [10] Video kompres: MP4 asli kalau browser support, else WebM transparan
   ================================================================ */
(function () {
  'use strict';
  ...
  const STORAGE_KEY = 'irgxy_iqc_prefs_v2';
  ...
  function savePrefs() {
    safe(() => localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      showWallpaper: State.showWallpaper, contact: State.contact, custom: State.custom
    })));
  }
  function loadPrefs() {
    safe(() => {
      ...
      if (typeof d.mode === 'string' && ['whatsapp','instagram','custom'].includes(d.mode)) {
        State.mode = d.mode;
      }
    });
  }
  ...
  function doRenderPreview() {
    ...
    /* Status bar */
    if (statusBar) statusBar.style.display = State.statusBar ? 'flex' : 'none';
  ...
  function initTabs() {
    ...
    if (customCard) customCard.style.display = (State.mode === 'custom') ? 'block' : 'none';
  }
  ...
  function releaseVideoPreview() {
    ...
    if (pv) { try { pv.pause(); } catch {} pv.removeAttribute('src'); try { pv.load(); } catch {} }
  }
  ...
  /* --- probeVideo v2.0: TIDAK return URL yang akan direvoke --- */
  ...
  /* --- Detect best MIME for MediaRecorder --- */
  ...
  async function compressWithCanvasRecorder(file, targetHeight, videoBitrate, audioBitrate, onProgress, signal) {
    ...
    try {
      ...
      cleanupAll.push(() => { try { v.pause(); } catch {} v.removeAttribute('src'); try { v.load(); } catch {} });
      ...
      /* ---- Audio (opsional, tidak boleh gagal total) ---- */
      let audioCtx = null, sourceNode = null, destNode = null;
      const stream = canvas.captureStream(30);
      cleanupAll.push(() => stream.getTracks().forEach(t => { try { t.stop(); } catch {} }));
      ...
      try {
        ...
        if (AC) {
          ...
          // Note: karena v.volume=0, tidak ada output ke speaker
      ...
      const drawFrame = () => {
        ...
        try { ctx.drawImage(v, 0, 0, outW, outH); } catch {}
        frameCount++;
      ...
      await new Promise(res => {
        ...
        // Safety timeout: durasi + 10 detik
      ...
      // Jalankan cleanup tambahan
      cleanupAll.forEach(fn => { try { fn(); } catch {} });
    }
  }
  ...
  function init() {
    try {
      ...
      if (window.AOS && typeof AOS.init === 'function') {
        try { AOS.init({ duration: 650, easing: 'ease-out-expo', once: true, offset: 30 }); } catch {}
      }
      console.log('✅ IQC Generator v2.0 STABLE siap');
    } catch (err) {
