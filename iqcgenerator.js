/* ================================================================
   ...
   Version: 5.10 (Bubble #444444 + Emoji Font iOS)
   ================================================================
   CHANGELOG v5.10:
     [FIX #1] DEFAULT_CM.bubbleColor → '#444444' (dari #1C1C1E)
     [FIX #2] Emoji font: iOS stack + font-variant-emoji:emoji
     [FIX #3] onclone: force bubble #444444 & emoji iOS font
     [FIX #4] Post-render force bg color (anti cache legacy)
   ================================================================ */
(function () {
  'use strict';
  ...
  /* v5.10 — Bubble & Emoji color (WAJIB #444444) */
  const BUBBLE_COLOR = '#444444';

  /* v5.10 — Emoji iOS font stack */
  ...
  const DEFAULT_CM = {
    ...
    messageTime: '17:25',
    messageType: 'in',
    /* v5.10 — Bubble #444444 (100% akurat) */
    bubbleColor: '#444444',
    textColor: '#FFFFFF',
    timeColor: '#8E8E93',
    reactions: ['👍','❤️','😂','😮','😢','🙏'],
  ...
  function loadPrefs() {
    safe(() => {
      ...
      if (d.contextmenu && typeof d.contextmenu === 'object') {
        ...
        if ('menuItems' in State.contextmenu) delete State.contextmenu.menuItems;
      }
  ...
  /* ================================================================
     v5.10 — RENDER CONTEXT MENU (Bubble #444444 + Emoji iOS)
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.10 — FORCE bubble color #444444 (abaikan legacy localStorage) */
    ...
    /* v5.10 — Sync State agar form & preview konsisten */
    ...
    /* v5.10 — Post-render force: pastikan background = #444444
       (mengalahkan cache CSS legacy) */
    requestAnimationFrame(() => {
      ...
      /* Force emoji font iOS pada semua emoji span */
  ...
  function initContextMenuForm() {
    ...
    /* v5.10 — Sync bubbleColor ke #444444 */
    ...
    /* v5.10 — Bubble color input di-lock ke #444444 */
    const colorInput = $('#iqcCmBubbleColor');
    if (colorInput) {
      colorInput.value = BUBBLE_COLOR;
      colorInput.addEventListener('input', () => {
        /* Paksa selalu #444444 apapun yang diubah user */
  ...
  function preloadContextMenuImage() {
    return new Promise((resolve) => {
      ...
      img.onload  = () => { console.log('[IQC v5.10] CM image loaded:', url); finish(); };
      img.onerror = () => { console.warn('[IQC v5.10] CM image gagal load:', url); finish(); };
  ...
  /* ================================================================
     v5.10 — CAPTURE CANVAS (Bubble #444444 + Emoji iOS)
     ================================================================ */
  async function captureCanvas(scale = 3) {
    ...
    await Promise.all([preloadBackgroundImage(), preloadContextMenuImage()]);
      ...
      return await html2canvas(frame, {
        ...
        onclone: (clonedDoc) => {
          try {
            ...
            /* ── 5) v5.10 FIX — FORCE bubble & reactions background #444444 ── */
            ...
            /* ── 6) v5.10 FIX — Force emoji iOS font pada semua emoji ── */
            ...
            clonedDoc.querySelectorAll('.iqc-cm-bubble-content, .iqc-bubble-content').forEach(el => {
            ...
            /* ── 6b) Force context menu image rendering ── */
            ...
            /* ── 7) Force text font stack ── */
            const targets = clonedDoc.querySelectorAll(
              '.iqc-cm-plus, .iqc-cm-bubble-time, .iqc-bubble-time, .iqc-date-pill'
            );
            targets.forEach(el => {
              el.style.fontFamily = `${TEXT_FONT}, ${EMOJI_FONT}`;
            ...
            killStyle.textContent = `
              *, *::before, *::after {
              ...
              /* v5.10 — Force bubble & reactions #444444 */
              .iqc-cm-bubble { background: ${BUBBLE_COLOR} !important; }
              .iqc-cm-reactions { background: ${BUBBLE_COLOR} !important; }
              /* v5.10 — Force emoji iOS font */
          ...
          } catch (e) { console.warn('[IQC onclone v5.10]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  function init() {
    try {
      loadPrefs();
      /* v5.10 — Paksa bubbleColor = #444444 setelah loadPrefs */
      State.contextmenu.bubbleColor = BUBBLE_COLOR;
      initTabs();
      initForm();
      initAvatarUpload();
      ...
      console.log('✅ IQC Generator v5.10 (Bubble #444444 + Emoji iOS) siap');
    } catch (err) {
  ...
  window.IQC = {
    ...
    preloadBackgroundImage, preloadContextMenuImage, captureCanvas
  };
})();