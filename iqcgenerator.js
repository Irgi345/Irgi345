/* ================================================================
   IQC GENERATOR — iqcgenerator.js (v5.2 — BACKGROUND IMAGE FIX)
   ================================================================
   CHANGELOG:
   v5.2 (Background Image Fix):
     - Tambah konstanta CHAT_BG_URL untuk background chat-bg
     - Simpan chatBgUrl ke State & localStorage
     - doRenderPreview() & renderContextMenuPreview() sekarang set
       background-image URL (cover, center, no-repeat, scroll)
     - Wallpaper OFF tidak lagi mengganti ke warna solid
     - Dark mode TIDAK menimpa background image
     - Preload image di init() dengan crossOrigin='anonymous'
     - captureCanvas(): useCORS:true, allowTaint:false
     - onclone(): JANGAN override background chat-bg
   v5.1:
     - Bubble context menu pakai <span> untuk content & time
     - Content inline, time absolute overlay (hug-content 100%)
   ================================================================ */
(function () {
  'use strict';
  ...
  /* ================= CONSTANTS ================= */
  // URL background chat (dipakai di CSS & JS, dan dipersist ke localStorage)
  const CHAT_BG_URL = 'https://ik.imagekit.io/IrgxyMods/2383af11-2091-4331-9887-6627d8b71d7c.png';
  const BG_FALLBACK_COLOR = '#0B141A'; // fallback jika gambar gagal load
  ...
  const State = {
    ...
    showWallpaper: true,
    /* === v5.2: URL background chat (dipersist) === */
  ...
  function savePrefs() {
    safe(() => localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      chatBgUrl: State.chatBgUrl,          /* v5.2 */
  ...
  function loadPrefs() {
    safe(() => {
      ...
      /* v5.2: validasi URL background (harus string non-kosong) */
  ...
  /* ================= v5.2: APPLY BACKGROUND CHAT-BG =================
     Fungsi terpusat untuk set background image chat-bg.
     Dipakai di doRenderPreview() dan renderContextMenuPreview().
     - cover, center, no-repeat, scroll (aman html2canvas)
     - background-color fallback tetap
     - tidak mengubah dimension / bubble / layout
  =================================================================== */
  function applyChatBackground(chatBg) {
    ...
    chatBg.style.backgroundAttachment = 'scroll'; // JANGAN fixed (rusak di html2canvas)
  ...
  function doRenderPreview() {
    ...
    /* === v5.2: class toggle hanya untuk overlay/tekstur, TIDAK ganti bg === */
    ...
    /* === v5.2: SET BACKGROUND IMAGE (BUKAN warna solid) === */
  ...
  /* ================= RENDER CONTEXT MENU PREVIEW (v5.2) ================= */
  function renderContextMenuPreview() {
    ...
    /* === v5.2: context menu mode juga pakai background image URL === */
    chatBg.classList.toggle('dark', isDark);
    chatBg.classList.remove('has-wallpaper', 'no-wallpaper');
    applyChatBackground(chatBg);      // <- set URL, cover, center, no-repeat, scroll
  ...
  async function captureCanvas(scale = 2) {
    ...
    return await html2canvas(frame, {
      useCORS: true,
      allowTaint: false,          /* v5.2: strict CORS, image dengan crossOrigin harus sukses */
      ...
      onclone: (clonedDoc) => {
        try {
          /* v5.2: JANGAN override background chat-bg di clonedDoc.
             Hanya nonaktifkan backdrop-filter untuk kompatibilitas html2canvas. */

          // chat-bg: pastikan background-image tetap ada
  ...
  function resetForm() {
    ...
    State.chatBgUrl = CHAT_BG_URL;   /* v5.2 */
  ...
  /* ================= v5.2: PRELOAD BG IMAGE ================= */
  function preloadChatBackground() {
    const bgImg = new Image();
    bgImg.crossOrigin = 'anonymous';   // penting agar tidak taint canvas
    bgImg.src = (State.chatBgUrl && String(State.chatBgUrl).trim()) || CHAT_BG_URL;
    bgImg.onload = () => {
      console.log('[IQC] BG image loaded:', bgImg.src);
      renderPreview(); // re-render agar html2canvas tahu gambar sudah siap
    };
    bgImg.onerror = () => {
      console.warn('[IQC] BG image gagal load:', bgImg.src);
      // Tetap render — fallback color akan muncul dari CSS
      renderPreview();
    };
  }
  ...
  function init() {
    try {
      ...
      initFaq();
      initKeyboard();
      preloadChatBackground();  /* v5.2: preload sebelum render */
      renderPreview();
      renderMessageList();
      ...
      console.log('✅ IQC Generator v5.2 (Background Image Fix) siap');
    } catch (err) {
