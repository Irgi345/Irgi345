/* ================================================================
   ...
   Version: 5.8.1 (PIXEL-PERFECT — Preview = Download 100%)
   ================================================================
   CHANGELOG v5.8.1:
     [FIX #1] menuItems TIDAK LAGI menyimpan field `icon:'reply'`, dll.
              Karena ikon sudah diganti <img src="${CM_ICON_URL}">
              Sekarang menuItems hanya: { label, danger }.
     [FIX #2] data-action di renderContextMenuPreview() dihasilkan
              otomatis dari slug(label) — aman XSS, unik, stabil.
     [FIX #3] loadPrefs() migrasi otomatis: legacy menuItems dari
              localStorage lama di-strip field `icon` agar tidak error.
     [FIX #4] Guard ketat di renderContextMenuPreview() → toleran
              terhadap data korup / legacy / undefined.
     [FIX #5] DEFAULT_CM.menuItems bersih — tidak ada lagi warisan
              Font Awesome. Semua item pakai satu URL gambar.
   ================================================================
   CHANGELOG v5.8:
     [FIX] Context Menu icon: Font Awesome <i> → <img> ImageKit
     [FIX] Preload gambar ikon (preloadContextMenuIcons)
     [FIX] onclone: paksa rendering <img> context menu
     [FIX] imageTimeout 30000ms
     [FIX] Anti-aliasing teks menu & bubble
     [FIX] Force solid bg .iqc-cm-menu (backdrop-filter tidak support)
     [FIX] Hapus menuIcons map
   ================================================================ */
(function () {
  'use strict';
  ...
  /* v5.8: URL tunggal untuk SEMUA ikon context menu */
  ...
  /* v5.8.1: slug generator untuk data-action (pengganti field icon) */
  ...
  /* ================= STATE ================= */
  /* v5.8.1 FIX: menuItems kini hanya { label, danger } —
     tidak ada lagi field icon yang usang. */
  const DEFAULT_CM = {
  ...
  /* v5.8.1: Normalisasi array menuItems dari sumber manapun
     (localStorage lama / user input). Hasil: { label, danger } saja.
     Mencegah error akibat field `icon` legacy atau struktur korup. */
  ...
  function loadPrefs() {
    safe(() => {
      ...
      if (d.contextmenu && typeof d.contextmenu === 'object') {
        ...
        /* v5.8.1: migrasi legacy menuItems — strip field `icon` usang */
  ...
  /* ================================================================
     RENDER CONTEXT MENU — v5.8.1 (IMAGE ICONS, NO `icon` FIELD)
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.8.1 FIX: menuItems kini HANYA { label, danger }.
       data-action dihasilkan dari slug(label). Ikon = <img CM_ICON_URL>. */
  ...
  /* ================================================================
     ...
     Wajib sebelum html2canvas agar ikon tidak blank.
     ================================================================ */
  function preloadContextMenuIcons() {
    return new Promise((resolve) => {
      ...
      img.onload = () => { console.log('[IQC v5.8.1] CM icon loaded:', CM_ICON_URL); finish(); };
      img.onerror = () => { console.warn('[IQC v5.8.1] CM icon gagal load:', CM_ICON_URL); finish(); };
      img.src = CM_ICON_URL;
  ...
  /* ================================================================
     v5.8.1 FIX — CAPTURE CANVAS (PIXEL-PERFECT)
     ================================================================ */
  async function captureCanvas(scale = 3) {
    ...
    /* v5.8: preload bg + ikon context menu paralel */
    ...
    try {
      return await html2canvas(frame, {
        ...
        onclone: (clonedDoc) => {
          ...
          } catch (e) { console.warn('[IQC onclone v5.8.1]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  /* ================================================================
     v5.8.1 — DOWNLOAD PNG (scale 3 / HD 4, retry 2x, quality 1.0)
     ================================================================ */
  ...
  function init() {
    try {
      ...
      /* v5.8.1: preload ikon context menu lebih awal */
      ...
      console.log('✅ IQC Generator v5.8.1 (Image Icons — Clean MenuItems) siap');
    } catch (err) {
