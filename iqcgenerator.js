/* ================================================================
   IQC GENERATOR — iqcgenerator.js (v5.3 — COLOR & STATUS MATCH)
   ================================================================
   CHANGELOG v5.3:
     - Force HIDE status bar + notch di mode Context Menu
       (frame class "cm-mode" → CSS hide ::before & .iqc-status-bar)
     - DEFAULT_CM disesuaikan 100% dengan gambar referensi:
         bubbleColor  : #1F1F1F  (abu gelap)
         textColor    : #FFFFFF  (putih)
         timeColor    : #8E8E93  (iOS gray subtle)
         menuItems    : Balas, Teruskan, Salin, Beri bintang,
                        Hapus (danger), Lainnya...
     - Ikon menu diperbarui: fa-reply, fa-share, fa-copy,
       fa-star, fa-trash, fa-ellipsis
     - Warna bubble/menu/reactions dipastikan match akurat
   CHANGELOG v5.2:
     - Background image URL dari imagekit (cover, center, no-repeat)
     - Wallpaper OFF tetap pakai background image
     - Preload image & onclone aman html2canvas
   v5.1:
     - Bubble context menu pakai <span> (hug-content 100%)
   ================================================================ */
(function () {
  'use strict';
  ...
  /* ================= STATE ================= */
  /* v5.3: DEFAULT_CM disesuaikan warna & menu 100% match gambar referensi */
  const DEFAULT_CM = {
    messageText: 'sadar diri muka lu tuh jelek',
    messageTime: '17:25',
    messageType: 'in',
    /* v5.3: warna abu gelap bubble chat iOS (match gambar) */
    bubbleColor: '#1F1F1F',
    textColor: '#FFFFFF',
    timeColor: '#8E8E93',
    reactions: ['👍','❤️','😂','😮','😢','🙏'],
    ...
    /* v5.3: menu items sesuai urutan gambar: Balas, Teruskan, Salin, Beri bintang, Hapus, Lainnya... */
    menuItems: [
  ...
  function doRenderPreview() {
    ...
    if (State.mode === 'contextmenu') {
      ...
      /* v5.3: mode Instagram — hapus class cm-mode (restore notch & status bar) */
  ...
  /* ================================================================
     v5.3: RENDER CONTEXT MENU — STATUS BAR & NOTCH DIHAPUS TOTAL
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.3: tambah class "cm-mode" → CSS akan hide notch (::before) + status bar */
    ...
    /* v5.3: FORCE HIDE status bar (SMARTFREN/notch/battery tidak tampil) */
    ...
    /* v5.3: reactions pill + tombol PLUS (match gambar) */
    ...
    /* v5.3: bubble hug-content — <span> inline + time overlay absolute */
    const bubbleHTML = `
    ...
    /* v5.3: ikon menu sesuai gambar (Balas, Teruskan, Salin, Bintang, Hapus, Lainnya) */
    const menuIcons = {
      star:'fa-star',
      reply:'fa-reply',
      forward:'fa-share',
      copy:'fa-copy',
      speak:'fa-comment-dots',
  ...
  function init() {
    try {
      ...
      console.log('✅ IQC Generator v5.3 (Color & Status Match) siap');
    } catch (err) {
