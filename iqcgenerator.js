/* ================================================================
   ...
   Version: 5.7 (PIXEL-PERFECT — Preview = Download 100%)
   ================================================================
   CHANGELOG v5.7:
     [FIX #1] Blur pada hasil download dihilangkan total:
         - Tambah preloadBackgroundImage() sebelum capture
         - Freeze semua animasi CSS saat capture (inject <style>)
         - onclone() force ALL computed styles (font, color, bg, border)
         - Solid fallback untuk backdrop-filter (tidak didukung html2canvas)
         - Force emoji font stack pada semua elemen
         - Force bubble color dari State (bukan computed)
         - Download PNG scale 3, HD scale 4 (dari 2/3)
         - toBlob quality 1.0 (dari 0.98)
         - Retry logic 2x untuk captureCanvas()
     [FIX #2] Posisi jam di bubble kini presisi kanan-bawah (via CSS Grid)
     [FIX #3] Default bubbleColor = #1C1C1E (iOS System Gray 6 Dark)
   ================================================================ */
(function () {
  'use strict';
  ...
  /* ================= STATE ================= */
  // v5.7 FIX: bubbleColor default = #1C1C1E (iOS System Gray 6 Dark)
  const DEFAULT_CM = {
    messageText: 'ikan hiu ngedek ngedek Indonesia nihhh dekkk',
    messageTime: '17:25',
    messageType: 'in',
    bubbleColor: '#1C1C1E',   // ← v5.7: iOS System Gray 6 Dark
    textColor: '#FFFFFF',
    timeColor: '#8E8E93',
    reactions: ['👍','❤️','😂','😮','😢','🙏'],
    darkTheme: true,
  ...
  function doRenderPreview() {
    ...
    // v5.7 FIX: reset padding dinamis sesuai mode
  ...
  /* ================================================================
     RENDER CONTEXT MENU — CLEAN BACKGROUND (v5.7)
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    // v5.7 FIX: hapus padding chat-messages agar dim overlay full-cover
    ...
    // v5.7 FIX: default bubble color = #1C1C1E
  ...
  /* ================================================================
     ...
     Wajib sebelum capture agar html2canvas tidak render bg kosong.
     ================================================================ */
  ...
  /* ================================================================
     v5.7 FIX — CAPTURE CANVAS (PIXEL-PERFECT)
     ================================================================ */
  async function captureCanvas(scale = 3) {
    ...
    // 2) Tunggu font (emoji + web font) siap
    try {
      if (document.fonts && document.fonts.ready) await document.fonts.ready;
    } catch (e) { /* noop */ }

    // 3) Freeze animations globally (agar capture presisi)
      ...
      return await html2canvas(frame, {
        ...
        onclone: (clonedDoc) => {
          try {
            ...
            /* ── 4) Force dim overlay (z-index & solid color) ── */
            ...
            /* ── 5) Kill backdrop-filter → SOLID fallback (html2canvas tidak support) ── */
            ...
            /* ── 6) Force bubble colors dari State (bukan computed) ── */
            ...
            /* ── 7) Force emoji + text font stack pada elemen konten ── */
            const targets = clonedDoc.querySelectorAll(
              ...
              '.iqc-cm-menu, .iqc-cm-item, .iqc-cm-item span, .iqc-cm-item i, ' +
            ...
            /* ── 8) Kill animations & transitions di clone ── */
          ...
          } catch (e) { console.warn('[IQC onclone v5.7]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  /* ================================================================
     v5.7 FIX — DOWNLOAD PNG (scale default 3, retry 2x, quality 1.0)
     ================================================================ */
  async function downloadPNG(scale = 3) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        }, 'image/png', 1.0); // v5.7: quality max
      });
  ...
  function initExportButtons() {
    // v5.7 FIX: PNG scale 3 (dari 2), HD scale 4 (dari 3)
  ...
  function init() {
    try {
      ...
      console.log('✅ IQC Generator v5.7 (Pixel-Perfect) siap');
    } catch (err) {
  ...
  window.IQC = {
    ...
    preloadBackgroundImage, captureCanvas
  };
})();