/* ================================================================
   ...
   Version: 5.9 (Menu Context = Gambar URL, Tanpa Text)
   ================================================================
   CHANGELOG v5.9:
     [BREAKING] Array DEFAULT_CM.menuItems DIHAPUS TOTAL.
     [NEW]     Digantikan oleh DEFAULT_CM.menuImage = CM_ICON_URL.
     [NEW]     renderContextMenuPreview() render 1 gambar utuh
               menggantikan seluruh menu text sebelumnya.
     [NEW]     preloadContextMenuImage() — wajib sebelum html2canvas.
     [FIX]     onclone: paksa rendering gambar menu context.
     [FIX]     Hapus semua referensi sanitizeMenuItems, slugify, menuIcons.
     [FIX]     Tidak ada crash walau localStorage lama masih ada menuItems.
   ================================================================ */
(function () {
  'use strict';
  ...
  /* v5.9 — URL gambar menu context (menggantikan seluruh menu text) */
  ...
  /* ================= STATE ================= */
  /* v5.9 BREAKING: menuItems DIHAPUS.
     Diganti single image URL: menuImage. */
  const DEFAULT_CM = {
    ...
    /* v5.9 — gambar menu context (URL tunggal) */
  ...
  function loadPrefs() {
    safe(() => {
      ...
      if (d.contextmenu && typeof d.contextmenu === 'object') {
        ...
        /* v5.9: pastikan menuImage selalu valid URL */
        ...
        /* v5.9: bersihkan field legacy yang sudah tidak dipakai */
  ...
  /* ================================================================
     v5.9 — RENDER CONTEXT MENU (MENU = GAMBAR URL)
     Menggantikan seluruh menu text .iqc-cm-item dengan satu gambar.
     ================================================================ */
  function renderContextMenuPreview() {
    ...
    /* v5.9 — MENU = GAMBAR TUNGGAL DARI URL */
  ...
  /* ================================================================
     v5.9 — PRELOAD CONTEXT MENU IMAGE (WAJIB)
     Gambar menu context harus ter-cache sebelum html2canvas.
     ================================================================ */
  function preloadContextMenuImage() {
    return new Promise((resolve) => {
      ...
      img.onload  = () => { console.log('[IQC v5.9] CM image loaded:', url); finish(); };
      img.onerror = () => { console.warn('[IQC v5.9] CM image gagal load:', url); finish(); };
  ...
  /* ================================================================
     v5.9 — CAPTURE CANVAS (PIXEL-PERFECT)
     ================================================================ */
      ...
      return await html2canvas(frame, {
        ...
        onclone: (clonedDoc) => {
          try {
            ...
            /* ── 6b) v5.9: FORCE context menu image rendering ── */
            clonedDoc.querySelectorAll('.iqc-cm-menu-image').forEach(img => {
              ...
              /* High-quality rendering untuk scale besar */
              img.style.imageRendering = 'auto';
              img.style.webkitBackfaceVisibility = 'hidden';
              img.style.backfaceVisibility = 'hidden';
              img.style.transform = 'translateZ(0)';
              /* Pastikan src tetap valid */
          ...
          } catch (e) { console.warn('[IQC onclone v5.9]', e); }
        }
      });
    } finally {
      freezeStyle.remove();
  ...
  /* ================================================================
     v5.9 — DOWNLOAD PNG (scale 3 / HD 4, retry 2x, quality 1.0)
     ================================================================ */
  ...
  function init() {
    try {
      ...
      console.log('✅ IQC Generator v5.9 (Menu Context = Gambar URL) siap');
    } catch (err) {
  ...
  /* ================= EXPOSE ================= */
  window.IQC = {
    State, CHAT_BG_URL, CM_MENU_IMAGE_URL,
